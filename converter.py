...
serviceNum = int(input("Speech-to-Text: 1\nText-to-Speech: 2\nChoose a number:"))
...
if(serviceNum == 1):
    ...
    config = config['textSpeech']

    api = IAMAuthenticator(config['apikey'])
    service = TextToSpeechV1(authenticator=api)

    service.set_service_url(config['url'])
    text = input('Write something: ')

    with open('./output.mp3', 'wb') as audio_file:
        result = service.synthesize(text, accept='audio/mp3', voice='en-GB_CharlotteV3Voice').get_result()
